...
fn main() {
    ...
    // We can also create a vector with initial values using the `vec!` macro
    ...
    // You can use `[]` or `.get()`
    // Using `[]` (panics if index is out of bounds)
    let third: &i32 = &v[2]; 
    ...
    for i in &mut v3 { // `&mut v3` gives mutable references
